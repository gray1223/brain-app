import { createClient } from "@/lib/supabase/server";
import { redirect } from "next/navigation";
import {
  format,
  subDays,
  endOfDay,
  startOfWeek,
  startOfMonth,
} from "date-fns";
import { LifeDashboard } from "@/components/life/life-dashboard";
import type {
  Profile,
  Todo,
  Note,
  CalendarEvent,
  JournalEntry,
  Habit,
  HabitCompletion,
  Reminder,
} from "@/types/database";

const PRIORITY_ORDER: Record<string, number> = {
  urgent: 0,
  high: 1,
  medium: 2,
  low: 3,
};

export default async function LifePage() {
  const supabase = await createClient();

  const {
    data: { user },
  } = await supabase.auth.getUser();

  if (!user) {
    redirect("/auth/login");
  }

  const now = new Date();
  const todayStr = format(now, "yyyy-MM-dd");
  const todayEnd = endOfDay(now).toISOString();
  const weekStart = startOfWeek(now, { weekStartsOn: 1 }).toISOString();

  const [
    { data: profile },
    { data: notes },
    { data: todos },
    { data: journalEntry },
    { data: events },
    { data: habits },
    { data: habitCompletions },
    { data: reminders },
    { data: recentJournals },
    capturesResult,
    totalNotesResult,
    completedTodosWeekResult,
    journalEntriesMonthResult,
    activeProjectsResult,
  ] = await Promise.all([
    // Profile
    supabase
      .from("profiles")
      .select("*")
      .eq("id", user.id)
      .maybeSingle(),

    // Recent notes (5)
    supabase
      .from("notes")
      .select("*")
      .eq("user_id", user.id)
      .eq("is_archived", false)
      .order("updated_at", { ascending: false })
      .limit(5),

    // Incomplete todos (10, sorted by priority)
    supabase
      .from("todos")
      .select("*")
      .eq("user_id", user.id)
      .eq("completed", false)
      .order("created_at", { ascending: false })
      .limit(20),

    // Today's journal entry
    supabase
      .from("journal_entries")
      .select("*")
      .eq("user_id", user.id)
      .eq("date", todayStr)
      .maybeSingle(),

    // Upcoming calendar events (5)
    supabase
      .from("calendar_events")
      .select("*")
      .eq("user_id", user.id)
      .gte("start_time", now.toISOString())
      .order("start_time", { ascending: true })
      .limit(5),

    // Active habits
    supabase
      .from("habits")
      .select("*")
      .eq("user_id", user.id)
      .eq("is_archived", false)
      .order("created_at", { ascending: true }),

    // Today's habit completions
    supabase
      .from("habit_completions")
      .select("*")
      .eq("user_id", user.id)
      .eq("completed_date", todayStr),

    // Active reminders (not dismissed, due today or earlier)
    supabase
      .from("reminders")
      .select("*")
      .eq("user_id", user.id)
      .eq("is_dismissed", false)
      .lte("remind_at", todayEnd)
      .order("remind_at", { ascending: true }),

    // Journal entries for streak calculation (last 90 days)
    supabase
      .from("journal_entries")
      .select("date")
      .eq("user_id", user.id)
      .gte("date", format(subDays(now, 90), "yyyy-MM-dd"))
      .order("date", { ascending: false }),

    // Unprocessed captures count
    supabase
      .from("captures")
      .select("id", { count: "exact", head: true })
      .eq("user_id", user.id)
      .eq("processed", false),

    // Quick stats: total notes
    supabase
      .from("notes")
      .select("id", { count: "exact", head: true })
      .eq("user_id", user.id)
      .eq("is_archived", false),

    // Quick stats: completed todos this week
    supabase
      .from("todos")
      .select("id", { count: "exact", head: true })
      .eq("user_id", user.id)
      .eq("completed", true)
      .gte("completed_at", weekStart),

    // Quick stats: journal entries this month
    supabase
      .from("journal_entries")
      .select("id", { count: "exact", head: true })
      .eq("user_id", user.id)
      .gte("date", format(startOfMonth(now), "yyyy-MM-dd")),

    // Quick stats: active projects count
    supabase
      .from("projects")
      .select("id", { count: "exact", head: true })
      .eq("user_id", user.id)
      .eq("status", "active"),
  ]);

  const unprocessedCaptureCount = capturesResult.count ?? 0;
  const totalNotesCount = totalNotesResult.count ?? 0;
  const completedTodosWeekCount = completedTodosWeekResult.count ?? 0;
  const journalEntriesMonthCount = journalEntriesMonthResult.count ?? 0;
  const activeProjectsCount = activeProjectsResult.count ?? 0;

  // Sort todos by priority, take top 10
  const sortedTodos = ((todos as Todo[] | null) ?? [])
    .sort(
      (a, b) =>
        (PRIORITY_ORDER[a.priority] ?? 2) - (PRIORITY_ORDER[b.priority] ?? 2)
    )
    .slice(0, 10);

  // Compute journal streak
  const journalDates = new Set(
    ((recentJournals as { date: string }[] | null) ?? []).map((j) => j.date)
  );
  let currentStreak = 0;
  for (let i = 0; i <= 90; i++) {
    const checkDate = format(subDays(now, i), "yyyy-MM-dd");
    if (journalDates.has(checkDate)) {
      currentStreak++;
    } else if (i === 0) {
      // Today not written yet is okay, keep checking
      continue;
    } else {
      break;
    }
  }

  return (
    <LifeDashboard
      profile={(profile as Profile) ?? null}
      recentNotes={((notes as Note[] | null) ?? [])}
      incompleteTodos={sortedTodos}
      todayJournal={(journalEntry as JournalEntry | null) ?? null}
      upcomingEvents={((events as CalendarEvent[] | null) ?? [])}
      habits={((habits as Habit[] | null) ?? [])}
      habitCompletions={((habitCompletions as HabitCompletion[] | null) ?? [])}
      activeReminders={((reminders as Reminder[] | null) ?? [])}
      currentStreak={currentStreak}
      unprocessedCaptureCount={unprocessedCaptureCount}
      quickStats={{
        totalNotes: totalNotesCount,
        completedTodosThisWeek: completedTodosWeekCount,
        journalEntriesThisMonth: journalEntriesMonthCount,
        activeProjectsCount: activeProjectsCount,
      }}
    />
  );
}
